...
BINANCE_API_KEY = os.getenv("BINANCE_API_KEY")  # Store in environment variables
BINANCE_API_SECRET = os.getenv("BINANCE_API_SECRET")  # Store in environment variables
...
# Getting prices ang historical data from Binance
def fetch_crypto_prices(symbol, interval='1h', limit=100):
    """
    Fetch historical price data for a cryptocurrency using Binance API.
    ...
    Returns:
        pd.DataFrame: A DataFrame with Date, Open, High, Low, Close, and Volume.
    """
    try:
...
def plot_crypto_price(price_data, crypto_symbol):
    """
    Plot the historical price chart for a cryptocurrency.

    Args:
        price_data (pd.DataFrame): DataFrame containing Date and Close price.
        crypto_symbol (str): Symbol of the cryptocurrency.
    ...
    try:
        ...
        logging.error(f"Error generating crypto price chart: {e}")
        return "Error generating crypto price chart."

#  Forecast crypto values
def forecast_crypto_prices(price_data, periods=30):
    """
    Forecast cryptocurrency prices using Prophet.
    
    Args:
        price_data (pd.DataFrame): DataFrame with 'Date' and 'Close' columns.
    ...
    try:
        ...
        df = price_data[['Date', 'Close']].rename(columns={'Date': 'ds', 'Close': 'y'})
...
def plot_forecast(forecast, historical_data):
    """
    Plot the historical and forecasted cryptocurrency prices.

    Args:
        forecast (pd.DataFrame): DataFrame with forecasted prices.
    ...
    try:
        ...
        # Layout
        fig.update_layout(
            title="Cryptocurrency Price Forecast",
            xaxis_title="Date",
        ...
        return "Error generating forecast plot."
...
def crypto_analysis():
    ...
    if request.method == 'POST':
        crypto_symbol = request.form.get('crypto_symbol').upper()
        ...
        symbol = f"{crypto_symbol}USDT"  # Binance format for USD trading pairs
        ...
        market_volume = price_data["Volume"].sum()  # Example: Summing volumes for analysis
        ...
        forecast = forecast_crypto_prices(price_data)
        if forecast is not None:
            forecast_chart = plot_forecast(forecast, price_data)
        else:
            forecast_chart = "Forecasting error."

        # Pass all required data to the template
        crypto_info = {"name": crypto_symbol, "price": latest_price}
